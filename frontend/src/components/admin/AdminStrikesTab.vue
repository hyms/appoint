  ...
  <v-card variant="outlined" class="admin-tab-card">
    ...
    <v-card-text>
      <v-alert type="warning" variant="tonal" rounded="md" class="mb-6">
        Strike management allows you to temporarily block patients from booking appointments due to
      ...
      >
        ...
        <template v-slot:item.isActive="{ item }">
          <v-chip
            ...
            size="small"
            :variant="item.isActive ? 'flat' : 'tonal'"
          >
        ...
        <template v-slot:item.actions="{ item }">
          ...
          <v-chip v-else size="small" color="grey" variant="flat" class="text-white"
            >Resolved</v-chip
          >
        </template>
      </v-data-table>
...
async function resolveStrike(strikeId: string, patientId?: string) {
  ...
  try {
    await strikesService.resolveStrike(strikeId, resolution.trim())
